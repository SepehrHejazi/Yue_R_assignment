...
```{r}
p <- maize_merged
p <- p[p$Chromosome %in%  c(1:10)]
p <- p[p$Position != 'multiple']
p <- p[!is.na(p$Position)]

ggplot(data = p, mapping = aes(x=factor(Chromosome,levels=c(1:10), y=as.numeric(Position)/1000000), color='blue') + geom_point(size=0.1) + facet_wrap(~ Chromosome) + facet_wrap(~ chromosome)
```
```{r}
ggplot(data = maize_merged, mapping = aes(x=factor(Chromosome,levels=c(1:10)), y=as.numeric(Position)/1000000), color='blue') + geom_point(size=0.1) 
```
...
```{r}
library(ggplot2)
as <- read.table('./data/snp_position.txt', header = T,row.names=1,sep='\t')
as <- as[as$Chromosome %in% c(1:10),]
as <- as[as$Position != "multiple",]
as <- as[!is.na(as$Position),]
ggplot() + 
  geom_point(data = as, aes(x=factor(Chromosome,levels=c(1:10)), y=as.numeric(Position)/1000000), color = '#B15BFF', shape = 95, size = 5, alpha = 0.2) + 
...
```{r}
maize_snp <- maize_merged %>% group_by(Chromosome) %>% mutate(Position, Binned=cut(Position,25,labels=F))
```
